...
=============================================================================
v5 — что изменено и почему. Все числа ниже измерены на
krisha_astana_baseline.csv (7047 объявлений аренды, 2054 группы зданий),
воспроизводятся через report_accuracy() и check_audit_trail() и печатаются
при каждом запуске с --baseline. Ни одно утверждение здесь не следует
принимать на слово: если цифра не сходится после правки, значит правка
что-то сломала.
=============================================================================
...
3. «ТРЕБУЕТ ПРОВЕРКИ» отвязано от незаполненного поля.
   Было: 502 из 504 срабатываний шли по is_thin_description (finish_type
   пуст у 40% рынка), по уверенности — 3. Вердикт про «ошибку когорты или
   скрытый дефект» на деле означал «продавец не заполнил поле».
...
5. Аудит-трейл сходится.
   ...
   (check_audit_trail) и печатается при каждом запуске: 7046/7046.
...
8. Отделка деградирует явно, а не молча.
   Раскладка 4198 чистовая / 2823 неизвестно / 26 черновая означала, что у
   черновой квартиры на весь город 25 сравнимых: когорты по 5 объявлений,
   всегда L6, и при этом полноценный вердикт. Теперь при нехватке сегмент
   расширяется, строка помечается (cohort_notes), медианный размер когорты
   у черновой 17.5 вместо 5, а уверенность честно падает (0.539).
...
Что осознанно НЕ тронуто и почему:
    ...
    * РУЧНАЯ ПРОВЕРКА ни разу не сработала: requires_manual_review равен
      False у всех 7047 строк. Ветка не проверена ничем и остаётся
      непроверенной — это не «работает», это «не запускалось».
...
# Finish state moves price/m² by a measurable amount that the prior used
# to ignore entirely: on the baseline, "finished" medians run 3.5% above
# "unknown" for 1-room flats, 6.7% for 2-room and 8.8% for 3-room. Since
# cohorts are already finish-homogeneous (rough is never mixed with
# finished), shrinking them all toward one finish-blind prior dragged
# unfinished flats up and finished ones down. Cells thinner than this
# fall back to the finish-blind version.
MIN_FOR_FINISH_CELL = 20
...
# IQR-based and plain stdev scale; all pairs, finish-matched pairs only,
# and buildings with >=5 listings only) it stayed inside [0.705, 0.750].
...
# Below this many same-finish listings citywide, the finish partition is
# not a market segment but a handful of rows. On this baseline the split
# is 4198 finished / 2823 unknown / 26 rough: a rough target had 25
# possible comparables in the entire city, its cohorts came out at a
# median size of 5, always from L6, and it still received a full verdict
# with no indication that the comparison had collapsed. Below the floor
# the finish filter is relaxed to the whole pool and the row is marked.
MIN_FINISH_POOL = 150
...
#
# The new thresholds are anchored to that measured error rather than to
# round numbers: the discount side starts above p70 of |diff_pct| and the
# overprice side just below p75. This produces far fewer verdicts, and
# the ones it produces survive the model's own uncertainty.
#
# Re-derive whenever the prediction error changes — see
# CONFIDENCE_TIER_HIGH below and report_accuracy() at the bottom.
FIND_THRESHOLD_HIGH_CONF = 0.18
FIND_THRESHOLD_MED_CONF = 0.22
FIND_THRESHOLD_LOW_CONF = 0.28

OVERPRICE_THRESHOLD_HIGH_CONF = -0.15
OVERPRICE_THRESHOLD_MED_CONF = -0.19
OVERPRICE_THRESHOLD_LOW_CONF = -0.25
...
# score_row). On this baseline that variable runs p10 = 0.257,
# p50 = 0.463, p90 = 0.604, and these boundaries split the rows about
# 39 / 37 / 24.
#
...
CONFIDENCE_TIER_HIGH = 0.50
CONFIDENCE_TIER_MED = 0.35
...
# wrong (bad finish_type match, tiny/noisy cohort, undeclared defect,
# contaminated radius match) than a sign that the number is real.
...
# triggered by this set and essentially none by low confidence. Since
# finish_type is empty for 2823 of 7047 rows (40%), the verdict in
# practice meant "the seller left a field blank", while its text told the
# reader "probably a bad cohort or a hidden defect". Those are different
# claims and only one of them was true.
#
# The two are now separated: statistical doubt about the BENCHMARK stays
# in the verdict, and incompleteness of the LISTING is reported in its own
# output column (data_completeness / review_flags) where it belongs.
...
def review_flags(target, warnings, cohort_notes=None):
    ...
    verdict unreadable: 502 of its 504 firings meant "finish_type is
    blank", not "this price looks wrong".
    """
    flags = []
...
def citywide_median_by_rooms_finish(pool):
    """Citywide median price/m² per (room count, finish bucket).

    citywide_median_by_rooms() pools finished, unfinished and unknown
    listings together. That blend is used in three places — the L2.5
    cross-room rescale ratio, the credibility prior, and the old L5 price
    band — and in all three the consumer is a finish-HOMOGENEOUS cohort,
    so the blend is systematically off for it. Cells below
    MIN_FOR_FINISH_CELL are omitted; callers fall back.
    """
    cells = defaultdict(list)
    for r in pool:
        if r.get("_price_m2") is not None and r.get("_rooms") is not None:
            cells[(r["_rooms"], r.get("_finish_bucket"))].append(r["_price_m2"])
    return {
        key: statistics.median(values)
        for key, values in cells.items()
        if len(values) >= MIN_FOR_FINISH_CELL
    }


def room_median_for(rooms, finish_bucket, citywide_median, finish_medians):
    """Citywide median for a room count, narrowed to the finish bucket
    when that cell is populated enough to be worth using."""
    if finish_medians:
        cell = finish_medians.get((rooms, finish_bucket))
        if cell:
            return cell
...
def prior_medians_by_rooms_class(pool, score_index):
    ...
    for r in pool:
        ...
        if cls is None:
            continue
        # Two granularities: with and without finish. The finer cell is
        # preferred when it is populated, the coarser one keeps the prior
        # defined for rare combinations.
        cells[(r["_rooms"], cls, r.get("_finish_bucket"))].append(r["_price_m2"])
        cells[(r["_rooms"], cls, None)].append(r["_price_m2"])

    out = {}
    for key, values in cells.items():
        floor_n = MIN_FOR_FINISH_CELL if key[2] is not None else MIN_FOR_PRIOR_CELL
...
def cohort_prior(
    rooms, building_class_label, finish_bucket, class_priors,
    citywide_median, finish_medians=None,
):
    ...
    if building_class_label is not None and class_priors:
        # Most specific first: room count x building class x finish.
        cell = class_priors.get((rooms, building_class_label, finish_bucket))
        if cell:
            return cell
        cell = class_priors.get((rooms, building_class_label, None))
        if cell:
            return cell
    return room_median_for(rooms, finish_bucket, citywide_median, finish_medians)
...
def target_price_segment(target, pool, q25=None, q75=None):
    """Target's OWN price bucket. Only used as a last-resort fallback when
    building_class() below has no data — see its docstring for why this is
    circular and should not be the primary source of truth for L3/L4/L5."""
...
def credibility_weight(n):
    """Shrinkage weight Z for partial pooling (review finding #2): how much
    weight a signal built from `n` listings gets in a blend, vs. whatever
    it's being blended against. Z -> 1 as n grows past FULL_CREDIBILITY_N;
    Z = 0.5 at n = CREDIBILITY_K.

    Used in score_row to blend the exact building-level cohort
    (`building_near`, computed in build_cohort regardless of whether it
    cleared its own level's threshold) into whatever wider level the
    cascade actually selected: base_price_m2 = Z*median(building_near) +
    (1-Z)*median(selected cohort) — literally the formula from the review.
    A building with 4 listings against a threshold of 5 no longer gets
    thrown away and replaced wholesale by a wider, noisier level; it keeps
    a proportional say in the final number instead of disappearing,
    undifferentiated, into whichever broader cohort ends up selected.
...
def _rescale_other_rooms(
    rows, target_rooms, target_finish, citywide_median, finish_medians=None,
):
    ...
    """
    # Both sides of the ratio are taken at the TARGET's finish bucket, so
    # the projection is "what would this flat cost per m² if it had the
    # target's room count", not "...and a different finish". The cohort is
    # finish-homogeneous by construction, so a finish-blind ratio was
    # importing an average across finishes into a cohort that has none.
    target_room_median = room_median_for(
        target_rooms, target_finish, citywide_median, finish_medians
    )
    ...
    for r in rows:
        ...
        if r["_price_m2"] is None:
            continue
        other_room_median = room_median_for(
            r["_rooms"], target_finish, citywide_median, finish_medians
        )
...
def collect_cohorts(
    ...
    class_priors=None, finish_medians=None, spatial_index=None,
    building_index=None,
):
    ...
    """
    # Finish matching runs once, up front, against the WHOLE pool: L1/2
    # needs finish-matched rows of OTHER room counts too, and deriving
    # those from an already room-filtered list would be empty by
    # construction. Rough and finished are never mixed; an unknown-finish
    # target may fall back to finished+unknown at lower confidence.
    target_finish = target["_finish_bucket"]
    notes = []

    def _finish_pool(buckets):
        return [
            r for r in pool
            if r["_finish_bucket"] in buckets and not same_target_id(r, target)
        ]

    if target_finish == "unknown":
        finish_matched = _finish_pool({"finished", "unknown"})
    else:
        finish_matched = _finish_pool({target_finish})

    # Graceful degradation instead of a silent collapse. On this baseline
    # the finish split is 4198 finished / 2823 unknown / 26 rough, so a
    # rough target had 25 possible comparables in the whole city: its
    # cohorts came out at a median size of 5, always from L6, and it still
    # got a confident verdict with nothing in the output saying the
    # comparison had failed. When the segment is too thin to be a market,
    # the filter is relaxed and the row is MARKED, so the widening is
    # visible rather than assumed away.
    same_rooms_available = sum(
        1 for r in finish_matched if r["_rooms"] == target["_rooms"]
    )
    if same_rooms_available < MIN_FINISH_POOL:
        widened = _finish_pool({"finished", "unknown", "rough"})
        if len(widened) > len(finish_matched):
            finish_matched = widened
            notes.append("когорта_без_фильтра_отделки")
    ...
    def within_radius(candidates, radius_km):
        ...
        had the finish/room/self filters applied to it.
        """
        if not has_coords:
            return []
    ...
    if target["_rooms"] is not None:
        l12_other_rooms = _rescale_other_rooms(
            [r for r in finish_matched if in_target_building(r)],
            target["_rooms"],
            target_finish,
            citywide_median,
            finish_medians,
    ...
    if has_coords:
        ...
        # The band is centred on the target's OWN (room x class x finish)
        # median, not on the citywide room median. Centring it citywide
        ...
        median_for_rooms = cohort_prior(
            ...
            target_finish, class_priors or {}, citywide_median,
            finish_medians,
        )
        if median_for_rooms:
...
def build_cohort(
    target, pool, citywide_median, score_index,
    target_score=None, target_n=0, area_slopes=None,
    class_priors=None, finish_medians=None,
    ...
    if "_finish_bucket" not in target:
    ...
    cohorts, stop_level, _notes = collect_cohorts(
        ...
        area_slopes, class_priors, finish_medians, spatial_index,
        building_index,
    )
...
# evidence than a small same-building one. "2b"/"2.5" are retained as
# aliases of the merged first cohort so that any external caller still
# passing the old level strings keeps working.
LEVEL_CONFIDENCE = {
    "1-2": 1.00,
    "2b": 0.95,
    "2.5": 0.80,
...
def value_from_diff(diff_pct):
    ...
    saturated at diff_pct = +-0.10, while the НАХОДКА thresholds start at
    0.10 — so literally every apartment that earned a НАХОДКА verdict
    scored exactly 100, and a 10% discount was indistinguishable from a
    45% one. Since value carries 65% of price_quality_score, the "best
    deals" list was in practice being ordered by photo count.
...
def verdict_from_diff(diff_pct, benchmark_confidence, target, warnings, robust_z=None):
    ...
    # to be an OR-branch here, and since finish_type is blank on 40% of
    # this market it accounted for 502 of 504 ТРЕБУЕТ ПРОВЕРКИ verdicts
...
def analyze_cohorts(
    ...
    target_finish=None, finish_medians=None,
):
    ...
    * The prior is the (room count x building class) median rather than
    ...
    infos = []
    prior = cohort_prior(
        target_rooms, building_class_label, target_finish,
        class_priors or {}, citywide_median, finish_medians,
    )
...
def score_row(
    target, pool, segments, citywide_median, score_index, q25, q75,
    soft_target=True, room_bounds=None, class_priors=None, area_slopes=None,
    finish_medians=None, spatial_index=None, building_index=None,
):
    """
    ...
    # Finish matching happens inside cohort collection, not here. This
    # pool only excludes self and baseline-ineligible rows.
    cohort_pool = [
        r for r in pool
        if not same_target_id(r, target)
    ...
    cohorts, stop_level, cohort_notes = collect_cohorts(
        ...
        finish_medians=finish_medians, spatial_index=spatial_index,
    ...
    cohort_infos = analyze_cohorts(
        ...
        target_finish=target["_finish_bucket"], finish_medians=finish_medians,
    )
    ...
    combined_conf = round(
        BENCHMARK_CONF_SHARE * benchmark_conf + DATA_CONF_SHARE * data_conf, 3
    )

    # The verdict THRESHOLD is chosen by benchmark_confidence alone, not
    # by the blend. combined_conf remains an output column, but mixing
    # data_confidence into the threshold decision was wrong on the
...
def report_accuracy(baseline_path, results=None):
    ...
    All three are leave-one-out, so the comparison is fair. Measured on
    krisha_astana_baseline.csv the numbers came out 0.160 / 0.125 / 0.106
    median absolute relative error: the five-line version captures about
    70% of the total available improvement, and everything else in this
    module buys the remaining 30%. That is worth knowing before adding
    the next mechanism, and it is why the v5 work went into calibration
    rather than into more modelling — a knob sweep over the class
    threshold, CREDIBILITY_K, FULL_CREDIBILITY_N, the L3 weight and
    disabling L5 entirely moved the median error by less than 0.003 in
    every direction, i.e. the residual is not reachable from here.
    """
...
def run(input_path, output_path, baseline_path=None, soft_target=True):
    ...
    # `segments` was computed here, threaded through score_row as a
    # positional parameter and never read by anything. The parameter is
    # kept for callers that still pass it; the computation is gone.
    segments = None
    citywide_median = citywide_median_by_rooms(usable_pool)
    finish_medians = citywide_median_by_rooms_finish(usable_pool)
    ...
    results = [
        score_row(
            r, usable_pool, segments, citywide_median, score_index, q25, q75,
