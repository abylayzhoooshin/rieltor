...
LIST_PROGRESS_FILE = "progress_list.json"
...
def _load_detail_rows():
    """
    Читает DETAIL_OUTPUT_CSV в {id: row}.

    Файл пишется _write_detail_csv() каждые 20 карточек внутри прогона, и
    раньше запись была неатомарной (прямая перезапись целевого файла).
    Если процесс убивали/падал ровно во время такой записи, файл мог
    остаться без валидного заголовка (пустой, обрезанный, без колонки
    "id") — и следующий прогон падал тут с KeyError, а весь slow-track
    переставал работать до ручного вмешательства.

    Теперь запись атомарна (см. _write_detail_csv), но старые битые файлы
    на диске это не чинит, поэтому чтение всё равно защищено: если
    заголовок не тот, что ожидается (нет "id"), файл считается битым —
    печатаем предупреждение и стартуем с чистого состояния (rows = {}),
    вместо падения всего процесса. Это безопасно: все id, которых не
    оказалось в существующей таблице, просто уйдут в "новые" и будут
    перекачаны заново.
    """
    rows = {}
    if os.path.exists(DETAIL_OUTPUT_CSV):
        with open(DETAIL_OUTPUT_CSV, "r", encoding="utf-8-sig", newline="") as f:
            reader = csv.DictReader(f)
            if not reader.fieldnames or "id" not in reader.fieldnames:
                print(
                    f"   ⚠️  {DETAIL_OUTPUT_CSV} повреждён или в старом формате "
                    f"(нет колонки 'id', заголовок: {reader.fieldnames}). "
                    f"Игнорирую файл, все id будут перекачаны заново."
                )
                return {}
            for row in reader:
                rid = row.get("id")
                if not rid:
                    continue  # битая строка без id — пропускаем, а не падаем
                rows[rid] = row
    return rows
...
def _write_detail_csv(rows_by_id):
    """
    Атомарная перезапись DETAIL_OUTPUT_CSV: сначала пишем во временный
    файл рядом, затем os.replace() на целевой путь. os.replace — атомарная
    операция на уровне ОС (POSIX и Windows), поэтому даже если процесс
    убьют/упадёт ровно в этот момент, DETAIL_OUTPUT_CSV останется либо
    старым валидным файлом, либо новым валидным — битого промежуточного
    состояния на диске не будет.
    """
    directory = os.path.dirname(os.path.abspath(DETAIL_OUTPUT_CSV)) or "."
    fd, tmp_path = tempfile.mkstemp(
        prefix=".krisha_astana_detail_", suffix=".csv.tmp", dir=directory
    )
    try:
        with os.fdopen(fd, "w", encoding="utf-8-sig", newline="") as f:
            ...
            writer.writeheader()
            writer.writerows(rows_by_id.values())
        os.replace(tmp_path, DETAIL_OUTPUT_CSV)
    except BaseException:
        # прогон прервали ровно во время записи — подчищаем temp-файл,
        # чтобы не копились .tmp-мусора, целевой файл при этом не тронут
...
async def run_detail_stage(session):
    ...
    ВАЖНОЕ ИЗМЕНЕНИЕ: карточка качается ТОЛЬКО для id, которых ещё нет в
    DETAIL_OUTPUT_CSV ("новые" для этого воркера) — им нужен полный набор
    полей (фото, площадь, описание и т.п.) для будущей оценки. Для id, что
    уже есть в детальной таблице, отдельный поход в карточку ради одной
    только актуальной цены больше не нужен — цена и так есть в list-скане
    (Уровень 1, .a-card__price), она просто патчится в существующую строку.
    Это тот же приём, что и в fast track, только тут применяется к уже
    накопленным полным записям, а не к голым id.
    """
    ...
    # Новые id — те, для которых ещё нет полной карточки в DETAIL_OUTPUT_CSV.
    # Только они реально идут в очередь на скачивание.
    new_ids = [i for i in list_ids if i not in existing_rows]
    known_ids = [i for i in list_ids if i in existing_rows]
    ...
    todo_ids = [i for i in new_ids if i not in done_ids_this_run]

    print(
        f"Всего ID в текущем списке: {len(list_ids)}, из них новых (нужна полная карточка): {len(new_ids)}, "
        f"уже скачано в этом незавершённом прогоне: {len(done_ids_this_run)}, "
        f"к скачиванию сейчас: {len(todo_ids)} (параллельно: {DETAIL_CONCURRENCY} воркеров); "
        f"известных (только патч цены, без запроса): {len(known_ids)}"
    )

    # Патчим цену известным id прямо из list-скана — бесплатно, без единого
    # detail-запроса. Остальные поля (фото/описание/etc.) остаются от
    # последней полной карточки — это "последнее известное состояние", а
    # не сегодняшний снимок; если понадобится периодически освежать и их —
    # это отдельная задача (TTL на стороне оркестратора), не здесь.
    now_iso = datetime.now(timezone.utc).isoformat(timespec="seconds")
    patched = 0
    for advert_id in known_ids:
        raw_price = list_price_by_id.get(advert_id)
        if raw_price in (None, ""):
            continue  # не удалось достать цену из списка в этом прогоне — не портим последнюю известную
        existing_rows[advert_id]["price"] = raw_price
        existing_rows[advert_id]["scraped_at"] = now_iso
        patched += 1
    if patched:
        print(f"   💲 Цена обновлена из list-скана для {patched} уже известных id (без detail-запросов)")
    ...
    save_progress(DETAIL_PROGRESS_FILE, progress)
    print(
        f"🎉 Уровень 2 завершён. Новых карточек скачано: {len(todo_ids)}, "
        f"известных id обновлено по цене: {patched} (без detail-запросов)"
    )
